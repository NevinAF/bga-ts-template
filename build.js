 ...
 * PackageTest implementation : © Nevin A Foster
 *
...
$machinestates = ${writer.stringify(statesJSON, true)};`
		...
		fs.writeFileSync('packagetest.action.php',
`<?php
${writer.fileSignature}
class action_packagetest extends APP_GameAction
{
	/** @var packagetest $game */
	...
	public function __default()
	{
		if (self::isArg('notifwindow')) {
			...
			$this->view = "packagetest_packagetest";
...
if (fs.existsSync('source/client/tsconfig.json')) {
	...
	const tsCommand = 'npx tsc -p source/client --outFile packagetest.js' + (builder.watch ? ' -w' : '');
	builder.execCommand(`Typescript compiler: client/define.ts => packagetest.js`, tsCommand);

	// const cookbookModuleRegex = /"@cookbook\/([^"]+)"/g;
	// const searchString = 'define("packagetest", [';
	// const importFixer = () => {
	// 	if (!fs.existsSync('packagetest.js'))
	// 		return;

	// 	const data = fs.readFileSync('packagetest.js', 'utf8');
	// 	const start = data.indexOf(searchString);
	// 	if (start !== -1) {
	// 		const end = start + searchString.length;
	// 		const result = data.substring(0, start) + 'define([' + data.substring(end);
	// 		fs.writeFileSync('packagetest.js', result, 'utf8');
	...
	// 		const importsContent = writer.fileSignature + imports.map(importName => `\n/// <reference path="../../../../bga-ts-template/typescript/cookbook/${importName}.ts" />`).join('');
	...
	// builder.watchCommand("Import Generator: source/client/build/imports.d.ts", 'packagetest.js', importFixer);
}

if (fs.existsSync('source/client/packagetest.scss')) {
	builder.execCommand(`SCSS compiler: client/packagetest.scss => packagetest.css`, `npx sass --no-source-map source/client/packagetest.scss packagetest.css` + (builder.watch ? ' --watch' : ''));
}

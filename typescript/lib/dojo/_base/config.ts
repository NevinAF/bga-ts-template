...
import _0 = require("../global");
import _1 = require("../has");
import _require = require("require");

var config: Partial<DojoJS._base.Config> = {};
var i = _require.rawConfig;
// @ts-ignore - this is a shallow object copy
for (let key in i) config[key] = i[key];
if (!config.locale && "undefined" != typeof navigator) {
	var a =
		navigator.languages && navigator.languages.length
			? navigator.languages[0]
			// @ts-ignore - userLanguage is not defined on Navigator
			: navigator.language || navigator.userLanguage;
	a && (config.locale = a.toLowerCase());
}

export = config as DojoJS._base.Config;